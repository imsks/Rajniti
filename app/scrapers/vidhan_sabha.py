...
class VidhanSabhaScraper:
        ...
        for row in rows:
            ...
            if len(cols) >= 4:
                ...
                if party_name:  # Skip empty rows
                    parties_data.append(
                        {
                            # Fields expected by tests / JSON outputs
    ...
    def _discover_constituency_links(self) -> List[Dict[str, str]]:
        """
        Discover constituency links from the index page.
        ...
        for a in soup.find_all("a", href=True):
            href = a.get("href") or ""
            m = re.search(r"candidateswise-([A-Za-z0-9]+)\.htm", href)
            if not m:
                continue
            constituency_code = m.group(1)
            name = (a.get_text() or "").strip() or constituency_code
