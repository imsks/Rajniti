...
def _patch_env(env_dict):
    ...
    return patch.dict("os.environ", env_dict, clear=False), patch("os.getenv", side_effect=_getenv)
...
class TestGetDatabaseUrl:
    ...
    def test_venv_local_swaps_postgres_to_localhost(self):
        ...
        from app.database.config import get_database_url
        with patch("os.getenv", side_effect=lambda k, d="": LOCAL_DOCKER_URL if k == "DATABASE_URL" else d), \
             patch("os.path.exists", return_value=False), \
             patch("app.database.config.socket.getaddrinfo", side_effect=socket_mod.gaierror("no DNS")):
    ...
    def test_supabase_url_passed_through_unchanged(self):
        ...
        from app.database.config import get_database_url
        with patch("os.getenv", side_effect=lambda k, d="": SUPABASE_URL if k == "DATABASE_URL" else d):
            url = get_database_url()
...
class TestGetConnectArgs:
    ...
    def test_venv_local_no_hostaddr_for_localhost(self):
        ...
        with patch("os.getenv", side_effect=_getenv), \
             patch("app.database.config.socket.getaddrinfo", return_value=[
    ...
    def test_returns_empty_when_dns_fails_completely(self):
        ...
        with patch("os.getenv", side_effect=_getenv), \
             patch("app.database.config.socket.getaddrinfo", side_effect=socket_mod.gaierror("no DNS")), \
             patch("app.database.config.socket.gethostbyname", side_effect=socket_mod.gaierror("no DNS")):
            args = get_connect_args()
            assert args == {}
    ...
    def test_fallback_to_gethostbyname(self):
        ...
        import socket as socket_mod
        with patch("os.getenv", side_effect=_getenv), \
             patch("app.database.config.socket.getaddrinfo", side_effect=socket_mod.gaierror("fail")), \
...
class TestInitEngine:
    ...
    def test_creates_engine_with_local_url(self):
        ...
        mock_sf = MagicMock()
        try:
            with patch("app.database.session.get_database_url", return_value=LOCAL_LOCALHOST_URL), \
                 patch("app.database.session.get_connect_args", return_value={"hostaddr": "127.0.0.1"}), \
                 patch("app.database.session.get_echo_mode", return_value=False), \
                 patch("app.database.session.create_engine", return_value=mock_engine) as ce, \
    ...
    def test_creates_engine_with_supabase_url(self):
        ...
        mock_sf = MagicMock()
        try:
            with patch("app.database.session.get_database_url", return_value=SUPABASE_URL), \
                 patch("app.database.session.get_connect_args", return_value={"hostaddr": "3.6.80.100"}), \
                 patch("app.database.session.get_echo_mode", return_value=False), \
                 patch("app.database.session.create_engine", return_value=mock_engine) as ce, \
                 patch("app.database.session.sessionmaker", return_value=mock_sf):
                result = m.init_engine()
    ...
    def test_returns_none_on_engine_creation_failure(self):
        ...
        try:
            with patch("app.database.session.get_database_url", return_value=LOCAL_LOCALHOST_URL), \
                 ...
                 patch("app.database.session.get_echo_mode", return_value=False), \
                 patch("app.database.session.create_engine", side_effect=Exception("boom")):
                assert m.init_engine() is None
...
class TestInitDb:
    ...
    def test_warns_on_transaction_pooler(self):
        ...
        try:
            with patch("app.database.session.get_database_url", return_value=pooler_url), \
...
@pytest.mark.unit
@pytest.mark.parametrize("label,database_url,is_docker,url_substr,expect_hostaddr", MATRIX)
class TestConnectionMatrix:
    """End-to-end config→engine creation for each combo."""

    def test_url_resolution(self, label, database_url, is_docker, url_substr, expect_hostaddr):
        import socket as socket_mod
        from app.database.config import get_database_url
    ...
    def test_connect_args(self, label, database_url, is_docker, url_substr, expect_hostaddr):
        import socket as socket_mod
        from app.database.config import get_connect_args
        ...
        with patch("os.getenv", side_effect=_getenv), \
             patch("os.path.exists", side_effect=_exists), \
             patch("app.database.config.socket.getaddrinfo", side_effect=_getaddrinfo), \
             patch("app.database.config.socket.gethostbyname", return_value="3.6.80.100"):
            args = get_connect_args()
            if expect_hostaddr:
                assert "hostaddr" in args, f"[{label}] expected hostaddr in {args}"

    def test_engine_creation(self, label, database_url, is_docker, url_substr, expect_hostaddr):
        ...
        try:
            with patch("os.getenv", side_effect=_getenv), \
                 patch("os.path.exists", side_effect=_exists), \
                 patch("app.database.config.socket.getaddrinfo", side_effect=_getaddrinfo), \
                 patch("app.database.config.socket.gethostbyname", return_value="127.0.0.1"), \
                 patch("app.database.session.create_engine", return_value=mock_engine) as ce, \
                 patch("app.database.session.sessionmaker", return_value=mock_sf):
                result = m.init_engine()
...
class TestResolveIpv4:
    def test_returns_ipv4_from_getaddrinfo(self):
        ...
        with patch("app.database.config.socket.getaddrinfo", return_value=[
            (2, 1, 6, "", ("10.0.0.1", 5432))
        ]):
    ...
    def test_falls_back_to_gethostbyname(self):
        import socket as socket_mod
        from app.database.config import resolve_ipv4
        with patch("app.database.config.socket.getaddrinfo", side_effect=socket_mod.gaierror), \
    ...
    def test_returns_empty_on_total_failure(self):
        ...
        from app.database.config import resolve_ipv4
        with patch("app.database.config.socket.getaddrinfo", side_effect=socket_mod.gaierror), \
             patch("app.database.config.socket.gethostbyname", side_effect=socket_mod.gaierror):
