...
Rajniti is a **pnpm + Turborepo monorepo**. The migration into it is in
progress, so two layouts coexist right now:

```
apps/
  api/       Flask modular monolith (uv) — deploys to GCP Cloud Run     [live]
  web/       Next.js frontend                                    [placeholder]
packages/
  tsconfig/        shared TypeScript config
  eslint-config/   shared ESLint config
frontend/    the Next.js app that actually ships today (npm)      [migrating]
app/         the Flask app that actually ships today              [migrating]
```

`apps/web` is an empty shell whose scripts are no-ops so the Turborepo task
graph stays green. **Frontend work still happens in `frontend/`** until the
migration commit moves it; backend work in `apps/api` is live. When in doubt,
follow the paths in the issue.

The frontend consumes [`@sutra_ui/ui`](https://www.npmjs.com/package/@sutra_ui/ui)
and `@sutra_ui/tokens` as published npm dependencies — **do not build, fork, or
vendor UI primitives.** If a primitive is missing, raise it against Sutra rather
than reimplementing it here.

Root-level commands (Turborepo fans them out across workspaces):

```bash
pnpm install       # JS deps for all workspaces
pnpm dev           # backing infra (chroma) + every app's dev task
pnpm dev:api       # Flask only — http://localhost:8000
pnpm lint          # ruff + import-linter (api), eslint (web)
pnpm typecheck     # pyright (api), tsc (web)
pnpm test          # every workspace's suite
```

The `make` targets above remain the supported path for the apps that haven't
moved yet; both work during the transition.

---

## Module boundaries (backend)

`apps/api` is a **modular monolith**. Modules (`promises`, `reps`, `rag`,
`agents`) are isolated and talk to each other only through a module's **public
facade**, passing **DTOs** — never by importing internals, ORM rows, or Chroma
handles.

The allowed dependency direction is enforced in CI by `import-linter`:

```
(promises, reps)  ->  rag  ->  agents
```

- `promises` and `reps` are siblings and **may not import each other**. Combine
  their data in the routes layer instead.
- `core` is shared infrastructure and may be imported by anyone.
- Cross-module imports go through the package root — `from app.reps import
  RepsService`, never `from app.reps.internal...`.

Needing a new cross-module dependency is a design discussion: open an issue
rather than working around the contract.
