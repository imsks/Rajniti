  ...
  return (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.includes('/users/user-1/politicians') && method === 'GET') {
      return Promise.resolve(
        okResponse({ success: true, data: saved }),
      )
    }
    ...
    if (url.includes('/users/user-1/politicians/') && method === 'DELETE') {
      ...
      const index = saved.findIndex((row) => row.politician_id === politicianId)
...
describe('user politicians API integration', () => {
  it('add then get returns saved MP', async () => {
    ;(global.fetch as jest.Mock).mockImplementation(createPoliticiansFetchRouter())

    const addResult = await userService.addUserPolitician('user-1', 'pol-1', 'MP')
  ...
  it('remove clears politician from subsequent get', async () => {
    ...
    const removeResult = await userService.removeUserPolitician('user-1', 'pol-1')
  ...
  it('get returns null when backend responds with 500', async () => {
    ;(global.fetch as jest.Mock).mockResolvedValueOnce({
      ...
      status: 500,
      json: () =>
        Promise.resolve({ success: false, error: 'Failed to fetch politicians' }),
    })
